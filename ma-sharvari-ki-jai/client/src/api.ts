// API base URL should be provided via VITE_API_BASE_URL in production
...
export async function apiFetch<T>(path: string, opts: RequestInit = {}, token?: string | null): Promise<T> {
  ...
  try {
    ...
    if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
      throw new Error('Network error: Unable to reach the API server. Check API base URL and connectivity.')
    }
    throw error
  }
}
